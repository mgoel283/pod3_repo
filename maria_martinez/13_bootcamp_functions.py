...
#It is a list of strings and you can look it up using type() method
print(type(books))
print('\n')
...
def check_out(string):
    if string in books:
        ...
        print(f"{string} is not in our avaliable book selection")
    print('\n')

# 1.4
# Check out 'SAPIENS' using the check_out function
checkOutBooked = input("please enter the book you would like to check out\n").upper()
check_out(checkOutBooked)
...
# Return: Not needed for this function
def check_in(string):
    if string not in books:
        books.append(string)
# 1.6
# Check in 'SAPIENS' using the check_in function
checkinBooked = input("please enter the book you would like to check in\n").upper()
check_in(checkinBooked)
...
def search_by_name(string):
    if string in books:
        print(f'{string} is Available')
    else:
        print(f'{string} is Not Available')

# 1.8
# Search for the book 'JUST MERCY'
bookTitle = input("Please type in the name of the book you are looking for:\n").upper()
print('\n')
search_by_name(bookTitle)
print('\n')
...
print(f"we have a list of size {len(books_with_details)} and within it we have {len(books_with_details)} dictonaries")
# 2.1
...
def count_books():
    print(f"We have  {len(books_with_details)} books")
# 2.2
...
def search_by_author(string):
    ...
    for i in range(len(books_with_details)):
        if string in books_with_details[i]['author']:
            booksByAuthor.append(books_with_details[i]['title'])
        else:
            continue
...
printList = search_by_author('Timothy Snyder')
print(f'these are books by Timothy Snyder we have in our selections:\n {printList}')
print('\n')